...
--
* *MUST* return `UCode.InvalidArgument` if the passed in URI is not a valid xref:../../../basics/uri.adoc[uProtocol URI].
--

[.specitem,oft-sid="dsn~discovery-findservices-error-notfound~1",oft-needs="impl,test"]
--
* *MUST* return `UCode.NotFound` if no service matching the URI was found.
--

[.specitem,oft-sid="dsn~discovery-findservices-error-permission~1",oft-needs="impl,test"]
--
* *MUST* return `UCode.PermissionDenied` if the caller is not permitted to invoke this API.
--
...
--
* *MUST* return `UCode.NotFound` if no topic matching the URI was found.
--
...
--
* *MUST* return `UCode.PermissionDenied` if the caller is not uDiscovery service in the correct recursive order (i.e. local -> domain -> central).
...
--
* *MUST* return `UCode.NotFound` if no topic matching the URI was found when the `UServiceTopic` is being removed (i.e. `ttl=0`).
--
...
--
* Cached data *MUST* be flush if `GetServiceTopics()` returns `UCode.NotFound`, this indicates the information is no longer present upstream. 
